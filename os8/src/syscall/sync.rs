...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    let tid = current_task()                                                                            // process identifier
                        .unwrap()
    ...
    if det {
        ...
        while let Some(tid2) = process_inner.mutex_alloc[mid] {
            if visited.contains(&tid2) {
                println!("========== deadlock pid: {}, tid: {}, mutex_id: {} =====", 
    ...
    drop(process);
    mutex.lock();
    // finish safety check and implement changes
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if det {
        ...
        while !all_finished && !all_released {
            ...
            // not_finihed = not_finished.difference(&finished).collect();
            all_finished = not_finished.is_empty();
        }
        if !not_finished.is_empty() {
            println!("--- deadlock pid:{}, tid:{}, sem_id: {}", process.pid.0, tid, sem_id);
            return -0xdead;
        }
    }
    drop(process_inner);
